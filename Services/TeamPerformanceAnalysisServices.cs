...
namespace microservices_team_performance_analysis.Services
{
    public class TeamPerformanceAnalysisServices
    {
        ...
        private readonly string _workTeamSearchUrl;

        public TeamPerformanceAnalysisServices(DBContext context, HttpClient httpClient, IConfiguration configuration)
        {
            _context = context;
            _httpClient = httpClient;
            _workTeamSearchUrl = Environment.GetEnvironmentVariable("SERVICE_WORK_TEAM_SEARCH_URL") 
                            ?? configuration["SERVICE_WORK_TEAM_SEARCH_URL"];
        }
        ...
        public async Task<bool> CheckIfWorkTeamExistsAsync(string codice)
        {
            ...
            var url = $"{_workTeamSearchUrl}/work-team/{codice}";
